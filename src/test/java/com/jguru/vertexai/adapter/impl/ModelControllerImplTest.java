...
public class ModelControllerImplTest {
  ...
  public void shouldGenerateContentSuccessfullyAndFormatResponse() {
    // Arrange
    ...
    when(useCase.execute("gemini.pro", "Hello, world!")).thenReturn("Generated content");
    when(presenter.presentSuccess("Generated content")).thenReturn("SUCCESS: Generated content");

    ModelController controller = new ModelControllerImpl(useCase, presenter);
    ...
    verify(useCase).execute("gemini.pro", "Hello, world!");
    verify(presenter).presentSuccess("Generated content");
  }

  @Test
  @DisplayName("Should handle exception from use case and present error")
  public void shouldHandleExceptionFromUseCaseAndPresentError() {
    // Arrange
    GenerateContentUseCase useCase = mock(GenerateContentUseCase.class);
    GenerateContentPresenter presenter = mock(GenerateContentPresenter.class);

    when(useCase.execute("gemini.pro", "Hello, world!")).thenThrow(new IllegalArgumentException("Invalid model alias"));
  ...
  public void shouldHandleNullModelAliasGracefully() {
    ...
    verify(useCase, never()).execute(anyString(), anyString());
  ...
  public void shouldHandleEmptyModelAliasGracefully() {
    // Arrange
    GenerateContentUseCase useCase = mock(GenerateContentUseCase.class);
    GenerateContentPresenter presenter = mock(GenerateContentPresenter.class);
