...
class AuthenticationConfigTest {
  ...
  void shouldRequireAuthenticationType() {
    assertThatThrownBy(() -> AuthenticationConfig.builder().build())
  ...
  void shouldRequireApiKeyForApiKeyType() {
    assertThatThrownBy(
        () -> AuthenticationConfig.builder().withType(AuthenticationType.API_KEY).build())
        .isInstanceOf(IllegalArgumentException.class).hasMessageContaining("apiKey");
  }

  @Test
  void shouldRequireProjectAndLocationForAdc() {
    assertThatThrownBy(() -> AuthenticationConfig.builder()
        .withType(AuthenticationType.SERVICE_ACCOUNT_ADC).build())
        .isInstanceOf(IllegalArgumentException.class).hasMessageContaining("projectId");
    assertThatThrownBy(() -> AuthenticationConfig.builder()
        .withType(AuthenticationType.SERVICE_ACCOUNT_ADC).withProjectId("project").build())
  ...
  void shouldRequireAllFieldsForExplicitKey() {
    assertThatThrownBy(() -> AuthenticationConfig.builder()
        .withType(AuthenticationType.SERVICE_ACCOUNT_EXPLICIT_KEY).build())
        .isInstanceOf(IllegalArgumentException.class).hasMessageContaining("saKeyFile");
  }

  @Test
  void shouldBuildValidConfigurations() {
    AuthenticationConfig apiKeyConfig = AuthenticationConfig.builder()
        .withType(AuthenticationType.API_KEY).withApiKey("key").build();
    assertThat(apiKeyConfig.getApiKey()).isEqualTo("key");

    AuthenticationConfig adcConfig = AuthenticationConfig.builder()
        .withType(AuthenticationType.SERVICE_ACCOUNT_ADC).withProjectId("project")
        .withLocation("region").build();
    assertThat(adcConfig.getProjectId()).isEqualTo("project");

    AuthenticationConfig explicitConfig = AuthenticationConfig.builder()
        .withType(AuthenticationType.SERVICE_ACCOUNT_EXPLICIT_KEY).withProjectId("project")
