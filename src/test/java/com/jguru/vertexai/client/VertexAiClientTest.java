...
class VertexAiClientTest {
  ...
  void shouldExtractProjectIdFromServiceAccountCredentials() throws Exception {
    ...
    // Verification of project ID extraction is implicit if it doesn't throw IllegalStateException
  }
  ...
  void shouldHandleApiKeyAuthentication() {
    ...
    // When & Then
    // We expect this to fail with an invalid API key, but to important thing is that
  ...
  void shouldHandleServiceAccountADC() {
    ...
    // When & Then
    // We expect this to fail with invalid credentials, but to important thing is that
  ...
  void shouldRouteToStandardVertexAiForModelWithoutSpecialFlags() throws IOException {
    ...
    // And: We stub to protected methods to prevent real network calls and return a mock result
  ...
  void shouldRouteToChatCompletionsWhenGoogleOpenAiProviderIsPresent() throws IOException {
    ...
    // Then: Verify that callChatCompletionsApi was called with to correct model name (not to
    // alias)
  ...
  void shouldAppend404HintForCallStandardVertexAi() throws IOException {
    ...
    // And: We stub the internal loadCredentials method to throw a 404-like exception
    ...
    // When & Then: The exception should contain a helpful hint (from callStandardVertexAi's catch block)
  ...
  void shouldAppend403HintForCallStandardVertexAi() throws IOException {
    ...
    // And: We stub the internal loadCredentials method to throw a 403-like exception
    // This allows us to test the catch block in callStandardVertexAi
    doCallRealMethod().when(spyClient).callStandardVertexAi(anyString(), anyString());
  ...
  void shouldUseDefaultLocationWhenNoRegionOverride() throws IOException {
    ...
    // Then: Verify that callStandardVertexAi was called (which uses the default location from authConfig)
    verify(spyClient, times(1)).callStandardVertexAi(eq("standard-model-name"), eq("a prompt"));
  }
  ...
  void shouldUseModelRegionOverrideWhenPresent() throws IOException {
    ...
    // Then: Verify that callStandardVertexAi was called (which internally uses resolveEffectiveLocation)
    // The region override "global" should be used instead of the default "us-central1"
  ...
  void shouldUseRegionalModelRegionOverride() throws IOException {
    ...
    // The region override "us-east5" should be used instead of the default "us-central1"
